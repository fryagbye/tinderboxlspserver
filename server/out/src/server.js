...
try {
    ...
    function isCompatible(targetType, valueType) {
        ...
        if (normTarget === 'string')
            return normValue === 'string'; // String accepts mostly string, maybe numbers convert automatically in TB? Enforce strict? User said "Proposal OK" which implied strict warning.
        if (normTarget === 'number' || normTarget === 'num')
            return normValue === 'number';
        if (normTarget === 'boolean' || normTarget === 'bool')
            return normValue === 'boolean';
        if (normTarget === 'color')
            return normValue === 'color' || normValue === 'string'; // Colors are often strings?
    ...
    const operatorFamilies = new Map();
    ...
    try {
        ...
        if (opCsvContent) {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                if (firstCol.toLowerCase() === 'name' && row[1].trim() === 'OpClass')
                    continue;
                ...
                if (parenIndex > 0)
                    label = name.substring(0, parenIndex).trim();
                const dotIndex = label.indexOf('.');
                if (dotIndex > 0) {
                    const family = label.substring(0, dotIndex);
                    const member = label.substring(dotIndex + 1);
                    if (!operatorFamilies.has(family))
                        operatorFamilies.set(family, []);
                    if (!operatorFamilies.get(family)?.includes(member))
                        operatorFamilies.get(family)?.push(member);
                    // Add family to keywords so it highlights
                    keywordNames.add(family);
                }
                let kind = node_1.CompletionItemKind.Function;
                if (label.startsWith('$'))
                    kind = node_1.CompletionItemKind.Variable;
                else if (row[3] === 'Property')
