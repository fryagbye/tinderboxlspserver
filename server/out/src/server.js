...
async function scanWorkspace(dirPath) {
    try {
...
async function indexFileForCache(filePath) {
    try {
        ...
        for (const tag of tags) {
            if (tag.content.trim().length > 0) {
                ...
                if (formatted.trim() !== tag.content.trim()) {
                    edits.push(node_1.TextEdit.replace(node_1.Range.create(doc.positionAt(tag.start), doc.positionAt(tag.end)), formatted.trim()));
                }
            }
        }
        return edits;
...
function formatActionCode(text, options) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!trimmed.startsWith('//')) {
            ...
            for (let j = 0; j < newLine.length; j++) {
                ...
                if ((char === '"' || char === "'")) {
                    if (inString === char) {
                        ...
                        if (current)
                            parts.push({ type: 'code', text: current });
                        current = char;
                        inString = char;
                    }
                    else {
            ...
            if (current)
                parts.push({ type: inString ? 'string' : 'code', text: current });
            newLine = parts.map(p => {
                if (p.type === 'code') {
                    let s = p.text;
...
connection.onPrepareRename(async (params) => {
    ...
    const tokens = tokenize(text);
    const targetToken = tokens.find(t => (t.type === 'Identifier' || t.type === 'Keyword') &&
        offset >= t.start && offset <= t.start + t.length);
    if (!targetToken)
        return null;
    return {
        range: node_1.Range.create(doc.positionAt(targetToken.start), doc.positionAt(targetToken.start + targetToken.length)),
...
connection.onRenameRequest(async (params) => {
    ...
    locations.forEach(loc => {
        if (!changes[loc.uri])
            changes[loc.uri] = [];
        changes[loc.uri].push(node_1.TextEdit.replace(loc.range, newName));
    });
    return { changes };
});
...
async function validateTextDocument(textDocument) {
    ...
    if (textDocument.languageId === 'tinderbox-export-code') {
        const findExportTags = (input, baseOffset) => {
            const results = [];
            let i = 0;
            while (i < input.length) {
                if (input[i] === '^') {
        ...
        const allTags = [];
        const processInputRecursively = (input, baseOffset) => {
        ...
        const processedTagStarts = new Set();
...
function performActionCodeValidation(text, textDocument, baseOffset, options = {}) {
    const diagnostics = [];
    // --- MASKING Step ---
    let maskedText = text;
    ...
    while ((m = varAssignPattern.exec(text))) {
        ...
        const funcUsage = new Map();
        const declaredFuncsInFile = new Set();
        ...
        for (let k = 0; k < tokens.length; k++) {
            ...
            if (t.type === 'Keyword' && t.value === 'function') {
                ...
                let m = k + 1;
                while (m < tokens.length && tokens[m].type === 'Whitespace')
                    m++;
                if (m < tokens.length && tokens[m].type === 'Identifier') {
                    ...
                    if (declaredFuncsInFile.has(name)) {
                        diagnostics.push({
                            severity: node_1.DiagnosticSeverity.Warning,
                            range: node_1.Range.create(textDocument.positionAt(tokens[m].start), textDocument.positionAt(tokens[m].start + tokens[m].length)),
                ...
                // var Name
                let m = k + 1;
                while (m < tokens.length && (tokens[m].type === 'Whitespace' || tokens[m].value === ':'))
                    m++; // Skip :type too
                if (m < tokens.length && tokens[m].type === 'Identifier') {
        ...
        for (const [name, def] of localUsage.entries()) {
            if (!def.used) {
                diagnostics.push({
                    ...
                    range: node_1.Range.create(def.pos, { line: def.pos.line, character: def.pos.character + def.length }),
...
connection.onDidChangeWatchedFiles(async (_change) => {
    ...
    for (const event of _change.changes) {
        ...
        if (event.type === 1) { // Created
            ...
            try {
                const docText = await fs.promises.readFile(vscode_uri_1.URI.parse(uri).fsPath, 'utf8');
                const doc = vscode_languageserver_textdocument_1.TextDocument.create(uri, 'tinderbox-action-code', 0, docText);
...
const tinderboxExportTags = new Map();
...
connection.onHover(async (textDocumentPosition) => {
    ...
    if (isExportCode) {
        ...
        const allTags = [];
        const collectRecursively = (input, baseOffset) => {
            const tags = findExportTags(input, baseOffset);
            for (const tag of tags) {
                allTags.push(tag);
                if (tag.tagContent)
                    collectRecursively(tag.tagContent, tag.contentStart);
            }
        };
    ...
    const tokens = tokenize(content);
    const targetIndex = tokens.findIndex((t) => offset >= t.start && offset <= t.start + t.length);
    let hoveredWord = '';
    let hoveredRange;
    if (targetIndex !== -1) {
        ...
        if (targetToken.type === 'Identifier' || targetToken.type === 'Keyword') {
            ...
            while (scanIdx >= 0 && tokens[scanIdx].type === 'Whitespace')
                scanIdx--;
...
function isDeclaration(tokens, identifierIndex, functionStartOffset = 0) {
    let j = identifierIndex - 1;
    while (j >= 0 && tokens[j].type === 'Whitespace')
        j--;
    if (j < 0)
    ...
    if (tokens[j].type === 'Identifier') {
        let k = j - 1;
        while (k >= 0 && tokens[k].type === 'Whitespace')
            k--;
        if (k >= 0 && tokens[k].value === ':') {
            k--;
            while (k >= 0 && tokens[k].type === 'Whitespace')
                k--;
            if (k >= 0 && tokens[k].value === 'var')
                return true;
        }
    }
    ...
    if (tokens[j].value === '(') {
        ...
        while (k >= 0 && tokens[k].type === 'Whitespace')
            k--;
        if (k >= 0 && (tokens[k].value === 'each' || tokens[k].value === 'eachLine'))
            return true;
    }
    // 4. function argument
    let depthCounter = 0;
    ...
    for (let k = identifierIndex - 1; k >= 0; k--) {
        if (tokens[k].start < functionStartOffset)
            break;
        if (tokens[k].value === ')')
            depthCounter++;
            ...
            if (depthCounter < 0) {
                let m = k - 1;
                while (m >= 0 && tokens[m].type === 'Whitespace')
                    m--;
    ...
    if (foundFunction && depthCounter < 0)
        return true;
    return false;
}
connection.onDefinition(async (params) => {
    ...
    for (const [uri, symbols] of workspaceSymbolCache.entries()) {
        if (processedUris.has(uri))
            continue; // Already checked open documents
...
async function getReferenceLocations(doc, offset) {
    const text = doc.getText();
    const tokens = tokenize(text);
    const targetToken = tokens.find(t => (t.type === 'Identifier' || t.type === 'Keyword') &&
        offset >= t.start && offset <= t.start + t.length);
    ...
    for (const t of tokens) {
        if (t.type === 'Identifier' && t.value === targetName) {
            if (t.start >= searchStart && t.start + t.length <= searchEnd) {
                references.push(node_1.Location.create(doc.uri, node_1.Range.create(doc.positionAt(t.start), doc.positionAt(t.start + t.length))));
            }
        }
    }
    ...
    if (!isLocal && allUserFunctionNames.has(targetName)) {
        for (const uri of workspaceFiles) {
            if (uri === doc.uri)
                continue;
            try {
                const content = await fs.promises.readFile(vscode_uri_1.URI.parse(uri).fsPath, 'utf8');
                if (content.includes(targetName)) {
                    const otherTokens = tokenize(content);
                    const otherDoc = vscode_languageserver_textdocument_1.TextDocument.create(uri, 'tinderbox-action-code', 0, content);
                    otherTokens.forEach(t => {
                        if (t.value === targetName) {
                            references.push(node_1.Location.create(uri, node_1.Range.create(otherDoc.positionAt(t.start), otherDoc.positionAt(t.start + t.length))));
                        }
                    });
                }
            }
...
connection.onDocumentHighlight(async (params) => {
    ...
    return locations.filter(loc => loc.uri === doc.uri).map(loc => ({
        range: loc.range,
        kind: node_1.DocumentHighlightKind.Text
    }));
});
    ...
    if (range.start.line === range.end.line && (range.start.character !== range.end.character)) {
        ...
        if (doc) {
            ...
            if (selectedText.trim().length > 0) {
                ...
                const lineStartOffset = doc.offsetAt(node_1.Position.create(range.start.line, 0));
                const lineEndOffset = doc.offsetAt(node_1.Position.create(range.start.line + 1, 0)) || text.length;
                ...
                codeActions.push({
                    ...
                    edit: {
                        changes: {
                            [params.textDocument.uri]: [
                                {
                                    range: node_1.Range.create(node_1.Position.create(range.start.line, 0), node_1.Position.create(range.start.line, 0)),
    ...
    if (range.start.line !== range.end.line || range.start.character !== range.end.character) {
        ...
        if (doc) {
            ...
            if (selectedText.trim().length > 0) {
                ...
                const lastLineText = doc.getText(node_1.Range.create(lastLine - 1, 0, lastLine - 1, 1000));
                const insertionPos = node_1.Position.create(lastLine, 0);
                    ...
                    edit: {
                        changes: {
                            [params.textDocument.uri]: [
                                ...
                                {
                                    range: node_1.Range.create(insertionPos, insertionPos),
                                    newText: newFuncDef
                                }
                            ]
                        }
...
async function getReferenceLocationsForName(targetName) {
    const references = [];
    for (const uri of workspaceFiles) {
        try {
            let content;
            ...
            if (openDoc) {
                ...
                content = await fs.promises.readFile(vscode_uri_1.URI.parse(uri).fsPath, 'utf8');
            }
            if (content.includes(targetName)) {
                const tokens = tokenize(content);
                const doc = openDoc || vscode_languageserver_textdocument_1.TextDocument.create(uri, 'tinderbox-action-code', 0, content);
                tokens.forEach((t, i) => {
                    if (t.type === 'Identifier' && t.value === targetName) {
                        if (!isDeclaration(tokens, i)) {
                            references.push(node_1.Location.create(uri, node_1.Range.create(doc.positionAt(t.start), doc.positionAt(t.start + t.length))));
                        }
                    }
                });
            }
...
async function getContainingFunction(location) {
    ...
    try {
        ...
        if (openDoc) {
            ...
            text = await fs.promises.readFile(vscode_uri_1.URI.parse(docUri).fsPath, 'utf8');
        }
    }
    catch (e) {
        return null;
    ...
    const tokens = tokenize(text);
    const doc = vscode_languageserver_textdocument_1.TextDocument.create(docUri, 'tbox', 0, text);
            ...
            if (braceDepth === 0 && functionStart !== -1) {
                if (offset >= functionStart && offset <= t.start + t.length) {
                    return {
                        name: functionName,
                        uri: docUri,
                        range: node_1.Range.create(doc.positionAt(functionStart), doc.positionAt(t.start + t.length))
                    };
                }
                functionStart = -1;
            }
    ...
    for (const ref of refs) {
        ...
        if (!callsByCaller.has(key)) {
            callsByCaller.set(key, {
                ...
                uri: ref.uri,
                range: caller ? caller.range : node_1.Range.create(0, 0, 0, 0),
                fromRanges: []
            });
        }
...
connection.languages.callHierarchy.onOutgoingCalls(async (params) => {
    ...
    try {
        ...
        if (openDoc)
            ...
            text = await fs.promises.readFile(vscode_uri_1.URI.parse(item.uri).fsPath, 'utf8');
    }
    catch (e) {
        return null;
    }
    const doc = documents.get(item.uri) || vscode_languageserver_textdocument_1.TextDocument.create(item.uri, 'tinderbox-action-code', 0, text);
